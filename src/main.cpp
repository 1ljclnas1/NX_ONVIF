...
#include "main.h"
#include "FoundOnvifDevices.h"
#include "OnvifUriFilter.h"
#include "../onvifDs/h_files/filters.h"
  ...
  for (l_frame = batch_meta->frame_meta_list; l_frame != NULL;
    ...
    if (num_rects > 0) {
        printf("Pipeline: %s \n Frame Number = %d Number of objects = %d "
               "ID_0: %d ID_1: %d \n",
               pipeline_name.c_str(),frame_meta->frame_num, num_rects, count_0, count_1);
    }
  }
...
static gboolean
start_rtsp_streaming (guint rtsp_port_num, guint updsink_port_num, guint64 udp_buffer_size, std::string ip)
{
  ...
  std::regex hostname_filter("((\\d{3})[.](?:\\d{0,3})[.](\\d{0,3})[.](\\d{0,3}))");
  std:: smatch hostname_m;
  regex_search(ip, hostname_m, hostname_filter);
  std::string username;
  std::string passwd;
  bool flag = false;
  for(auto i : cnf["camera_auth"]){
      for(auto j : i.second){
          if(j.second.as<std::string>() == hostname_m.str(1)){
             username = i.second["result_username"].as<std::string>();
             passwd = i.second["result_passwd"].as<std::string>();
             flag = true;
          }
      }
  }
  if(!flag){
      username = cnf["camera_auth"]["default"]["result_username"].as<std::string>();
      passwd = cnf["camera_auth"]["default"]["result_passwd"].as<std::string>();
  }
  std::string params = "/ds/username=" + username + ",passwd="+ passwd;
  gst_rtsp_mount_points_add_factory (mounts, params.c_str(), factory);
  ...
  g_print
      ("\n *** DeepStream: Launched RTSP Streaming at rtsp://localhost:%d/ds/username=[username],passwd=[password] ***\n\n",
      rtsp_port_num);
...
int create_pipelines(std::vector<std::string> addrs, const ConfigParser& config_parser){
    if(addrs.size()==0) return 0;

    g_print("creating pipelines\n");

    std::vector<NvDsPipeline*> pipelineList;
    for(int k = 0;k < addrs.size();++k){
        if(addrs[k].length() == 0) continue;
        appCtx[k] = (AppCtx *) g_malloc0 (sizeof (AppCtx));

        NvDsPipeline *pipeline = &appCtx[k]->pipeline;
        ...
        pipeline->bus_id = gst_bus_add_watch (bus, bus_callback, appCtx[k]);
        ...
        /* create rtsp src bin */
        printf("%s: %u\n", "start createing source bins", 0);
        GstPad *sinkpad, *srcpad;
        gchar elem_name[50];
        g_snprintf (elem_name, sizeof (elem_name), "src_sub_bin%d", 0);
        pipeline->multi_src_bin.sub_bins[0].bin = gst_bin_new (elem_name);
        if (!pipeline->multi_src_bin.sub_bins[0].bin) {
            NVGSTDS_ERR_MSG_V ("Failed to create '%s'", elem_name);
            goto done;
        }

        pipeline->multi_src_bin.sub_bins[0].bin_id = pipeline->multi_src_bin.sub_bins[0].source_id = 0;
        pipeline->multi_src_bin.live_source = TRUE;
        pipeline->multi_src_bin.sub_bins[0].eos_done = TRUE;
        pipeline->multi_src_bin.sub_bins[0].reset_done = TRUE;


        if(!create_rtsp_src_bin (0, (gchar*)addrs[k].c_str(), &pipeline->multi_src_bin.sub_bins[0])){
            g_printerr ("Failed to create source bin. Exiting.\n");
        }
        gst_bin_add (GST_BIN (pipeline->multi_src_bin.bin), pipeline->multi_src_bin.sub_bins[0].bin);

        if (!link_element_to_streammux_sink_pad (pipeline->multi_src_bin.streammux,
                                                 pipeline->multi_src_bin.sub_bins[0].bin, 0)) {
            NVGSTDS_ERR_MSG_V ("while create pipeline for %s \n \
                               source %d cannot be linked to mux's sink pad %p\n", addrs[k],0, pipeline->multi_src_bin.streammux);
            goto done;
        }
        ...
        for(auto i : config_parser.m_source_list){
            if(addrs[k].find(i.m_hostname) != string::npos){
                g_object_set (G_OBJECT (pgie), "config-file-path", i.m_model.c_str(), NULL);
                found_hostname = true;
                break;
            }
        }
        if(!found_hostname) {g_object_set (G_OBJECT (pgie), "config-file-path", config_parser.m_source_list[0].m_model.c_str(), NULL);}
...
main (int argc, char *argv[]){
    ConfigParser config_parser("../config/config.yaml");
    nvds_version(&nvds_lib_major_version, &nvds_lib_minor_version);
    gst_init (&argc, &argv);
    std::vector<std::string> global_rtsp;
    loop = g_main_loop_new (NULL, FALSE);

    while(true){
        std::vector<std::string> rtsp;
        FoundOnvifDevices found("../config/config.yaml");
        found.probe(rtsp);
        cout << "probe" <<endl;
        std::set<std::string> rtsp_set;
        for(auto i : rtsp){
            rtsp_set.emplace(i);
        }
        rtsp.clear();
        ...
        OnvifUriFilter check("../config/filters.yaml");
        std::vector<std::string> clean_rtsp = check.filter(rtsp);
        std::vector<std::string> add_addrs;

        for (int i = 0;i < clean_rtsp.size();i++){
            if(global_rtsp.size() == 0){
                global_rtsp = clean_rtsp;
                add_addrs = clean_rtsp;
                break;
            }
            if(find(global_rtsp.begin(), global_rtsp.end(),clean_rtsp[i]) == global_rtsp.end()) {
                add_addrs.push_back(clean_rtsp[i]);
                global_rtsp.push_back(clean_rtsp[i]);
            }
        }
        std::thread t(create_pipelines, add_addrs, config_parser);
        t.detach();
    }
